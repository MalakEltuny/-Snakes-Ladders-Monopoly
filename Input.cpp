	...
	else	
	{
		if (UI.InterfaceMode == MODE_PLAY)
		{
			...
			if (y >= 0 && y < UI.ToolBarHeight)
			{
				...
				case ITM_EXIT_PLAY: return EXIT_PLAY;///TODO:
...
CellPosition Input::GetCellClicked() const
{
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		if ( y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			cellPos.SetVCell((floor(y / UI.CellHeight))-1);
			cellPos.SetHCell(floor(x / UI.CellWidth));///TODO: SetHCell and SetVCell of the object cellPost appropriately
