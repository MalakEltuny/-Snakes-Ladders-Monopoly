...
{
	int used = cellPos.VCell();
	int total = ((used) * UI.CellHeight)+UI.ToolBarHeight;///TODO: implement the following function as described in Output.h file

	return total; // this line should be changed with your implementation
}
...
void Output::DrawCardNumber(const CellPosition & cellPos, int cardNum) const
{
	...
	pWind->DrawInteger(x, y, cardNum);///TODO: Draw the Integer the representing the "cardNum" in the location (x,y)
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_COPY_CARD] = "images\\download.jpg";
	MenuItemImages[ITM_CUT_CARD] = "images\\cut card.jpg";
	MenuItemImages[ITM_PASTE_CARD] = "images\\paste.jpg";
	MenuItemImages[ITM_DELETE_GAME_OBJECT] = "images\\deleted.jpg";
	MenuItemImages[ITM_SAVE_GRID] = "images\\save grid.jpg";
	MenuItemImages[ITM_OPEN_GRID] = "images\\open grid.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\roll dice.jpg";
...
void Output::PrintPlayersInfo(string info)
{
	...
	pWind->DrawString(x, y, info);///TODO: Draw the string "info" in the specified location (x, y)
...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const
{
	...
	pWind->DrawRectangle(cellStartX, cellStartY+UI.ToolBarHeight, cellStartX + UI.CellWidth, cellStartY - UI.CellHeight+UI.ToolBarHeight);///TODO: Draw the Cell Rectangle using the appropriate coordinates
...
void Output::DrawPlayer(const CellPosition & cellPos, int playerNum, color playerColor) const
{
	...
	int yp = cellStartY + ySpace + radius + 2;
	if (playerNum == 2 || playerNum == 3)
		yp += radius + 2 + radius; // because playerNum 2 and 3 are drawn in the second row of circles

	// Calculate the Y coordinate of the center of the player's circle (based on playerNum)
	int xp = cellStartX + UI.LadderXOffset + radius + 4; // UI.LadderXOffset is used to draw players' circles 
														// AFTER the ladder start vertical line (assuming there is a ladder)
													    // for not overlapping with ladders
	if (playerNum == 1 || playerNum == 3)
		xp += radius + 2 + radius; // because playerNum 1 and 3 are drawn in the second column of circles
	pWind->SetBrush(playerColor);
	pWind->DrawCircle(xp, yp, radius);///TODO: Draw the player circle in center(x,y) and filled with the playerColor passed to the function
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if (!((toCell.VCell() < fromCell.VCell()) && (toCell.HCell() == fromCell.HCell())))
		return;///TODO: Validate the Cell Position (Must be Vertical Cells AND toCell above fromCell, otherwise, Do NOT draw)
	...
	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);///TODO: Set pen color and width using the appropriate parameters of UI_Info object (UI)



	pWind->DrawLine(x12, y1, x12, y2);///TODO: Draw The First Vertical Line (The Left Line) in the appropriate coordinates
	...
	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);///TODO: Set pen color and width using the appropriate variables of UI_Info object (UI)



	pWind->DrawLine(x34, y1, x34, y2);///TODO: Draw The Second Vertical Line (The Right Line) in the appropriate coordinates
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if (!((toCell.VCell() > fromCell.VCell()) && (toCell.HCell() == fromCell.HCell())))
		return;///TODO: Validate the fromCell and toCell (Must be Vertical and toCell is below fromCell otherwise do NOT draw)
	...
	int yup = fromStartY + UI.CellHeight/2;
	int ydown = toStartY + UI.CellHeight/2;

	pWind->SetPen(UI.SnakeColor, UI.SnakelineWidth);///TODO: Set pen color and width from the appropriate variables of the UI_Info object (UI)



	pWind->DrawLine(x12, yup, x12, ydown);///TODO: Draw the Line representing the Snake BOdy
	...
	int px1 = x12, py1 = yup;
	int px2 = x12 - xChange, py2 = yup + yChange;
	int px3 = x12 + xChange, py3 = py2;
	int px4 = x12, py4 = yup - yChange;///TODO: Set the coordinates of the 4 points of the Polygon
	//       Check the snakes drawn in the project document and draw it the same way
	int *arrX =new int[5] { px2, px1,px3, px1, px2 };
	int *arrY =new int[5] { py1, py4, py1, py2,py1 };
	
	pWind->DrawPolygon(arrX, arrY, 5);///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way
